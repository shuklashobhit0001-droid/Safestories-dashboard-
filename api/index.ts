...
async function handleTherapistDetails(req: VercelRequest, res: VercelResponse) {
  ...
  `, [name]);
  res.json({ clients: clientsResult.rows, appointments: appointmentsResult.rows });
}
...
async function handleTherapistStats(req: VercelRequest, res: VercelResponse) {
  ...
  res.json({
    ...
    upcomingBookings: upcomingResult.rows.map(booking => ({
      client_name: booking.client_name, therapy_type: booking.session_name, mode: booking.mode, session_timings: booking.session_timings
    }))
  });
}

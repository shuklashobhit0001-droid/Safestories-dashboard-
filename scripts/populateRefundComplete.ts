...
async function populateRefundTable() {
  try {
    ...
    console.log('🔄 Step 2: Adding sessions to appointment_table...\n');
    await pool.query(`
      INSERT INTO appointment_table (session_id, client_id, session_name, session_date)
      SELECT DISTINCT booking_id, invitee_id, booking_resource_name, booking_start_at
      FROM bookings WHERE booking_status IN ('cancelled', 'canceled')

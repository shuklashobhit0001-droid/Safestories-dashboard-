...
async function setupNotificationTriggers() {
  console.log('=== Setting Up Automatic Notification Triggers ===\n');

  try {
    // Check existing triggers
    console.log('1. Checking existing triggers...');
    const existingTriggers = await pool.query(`
      SELECT trigger_name, event_manipulation, event_object_table
      FROM information_schema.triggers
      WHERE event_object_table IN ('bookings', 'client_transfer_history', 'client_session_notes')
    `);
    
    if (existingTriggers.rows.length > 0) {
      console.log('Found existing triggers:');
      existingTriggers.rows.forEach(t => {
    ...
    console.log('2. Creating booking notification function...');
    await pool.query(`
      ...
      BEGIN
        ...
        ELSIF (TG_OP = 'UPDATE') THEN
          -- CANCELLED: Notify admins and therapist
          ...
          -- RESCHEDULED: Notify admins and therapist
          ...
          -- REFUND REQUESTED: Notify admins only
          ...
          -- REFUND COMPLETED: Notify admins only
      ...
      $$ LANGUAGE plpgsql;
    `);
    console.log('✓ Booking notification function created\n');

    // Create notification function for client transfers
    console.log('3. Creating client transfer notification function...');
    await pool.query(`
      ...
      BEGIN
        IF (TG_OP = 'INSERT') THEN
          ...
          -- Notify old therapist (transfer out)
          ...
          -- Notify new therapist (transfer in)
      ...
      $$ LANGUAGE plpgsql;
    `);
    console.log('✓ Client transfer notification function created\n');

    // Create notification function for session notes
    console.log('4. Creating session notes notification function...');
    await pool.query(`
      ...
      $$ LANGUAGE plpgsql;
    `);
    console.log('✓ Session notes notification function created\n');

    // Drop existing triggers
    console.log('5. Removing old triggers...');
    ...
    // Create new triggers
    console.log('6. Creating new triggers...');
    await pool.query(`
    ...
    console.log('=== ✅ SUCCESS ===\n');
    console.log('Automatic notifications enabled for BOTH Admin & Therapist!\n');
    console.log('📋 Admin Notifications:');
    console.log('  ✓ New bookings');
    console.log('  ✓ Cancelled bookings');
    console.log('  ✓ Rescheduled bookings');
    console.log('  ✓ No-show alerts');
    console.log('  ✓ Client transfers');
    console.log('  ✓ Session notes submitted');
    console.log('  ✓ Refund requests');
    console.log('  ✓ Refund completed\n');
    
    console.log('👨‍⚕️ Therapist Notifications:');
    console.log('  ✓ New bookings assigned');
    console.log('  ✓ Cancelled bookings');
    console.log('  ✓ Rescheduled bookings');
    console.log('  ✓ Client transfer in');
    console.log('  ✓ Client transfer out\n');
